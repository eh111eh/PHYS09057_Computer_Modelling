...
class Metropolis:
    def __init__(self, likelihood_function, initial_params, step_sizes, num_samples, burn_in=1000):
        ...
        self.samples = []
        self.accepted = 0  # Track accepted proposals
        self.total_steps = num_samples + burn_in  # Total MCMC steps
    ...
    def run(self):
        self.accepted = 0  # Reset accepted count
        ...
        for i in range(self.total_steps):
            ...
            if np.log(np.random.rand()) < log_alpha:
                ...
                self.accepted += 1  # Count accepted proposals
            
            if i >= self.burn_in:
                self.samples.append(self.current_params.copy())

        # Correct acceptance rate calculation
        self.acceptance_rate = self.accepted / self.total_steps
